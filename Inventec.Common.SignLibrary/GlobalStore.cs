...
namespace Inventec.Common.SignLibrary
{
    public class GlobalStore
    {
        ...
        private static List<EMR.EFMODEL.DataModels.EMR_SIGNER> EMR_SIGNERs;
        ...
        public static EMR.EFMODEL.DataModels.EMR_SIGNER GetByLoginName(string loginName)
        {
            ...
            try
            {
                ...
                {
                    //lấy danh mục người ký lưu RAM
                    if (EMR_SIGNERs == null)
                    {
                        EMR_SIGNERs = new Api.EmrSigner().Get();
                        if (EMR_SIGNERs != null)
                        {
                            EMR_SIGNERs = EMR_SIGNERs.OrderBy(o => o.USERNAME).ThenBy(o => o.NUM_ORDER).ToList();
                        }
                    }

                    //lấy người ký theo tài khoản không phân biệt hoa thường
                    if (EMR_SIGNERs != null && EMR_SIGNERs.Count > 0)
                    {
                        data = EMR_SIGNERs.FirstOrDefault(o => o.LOGINNAME.ToLower() == loginName.ToLower());
                    }

                    //không có dữ liệu trong RAM thì gọi API
                    if (data == null)
                    {
                        EMR.Filter.EmrSignerFilter filter = new EMR.Filter.EmrSignerFilter();
                        filter.LOGINNAME__EXACT = loginName;
                        List<EMR.EFMODEL.DataModels.EMR_SIGNER> datas = new Api.EmrSigner().Get(filter);
                        data = datas != null ? datas.Where(o => o.LOGINNAME == loginName).FirstOrDefault() : null;

                        //nếu lấy được dữ liệu từ API thì thêm vào RAM.
                        if (data != null && EMR_SIGNERs != null)
                        {
                            EMR_SIGNERs.Add(data);
                        }
                    }
                }
            }
